...
> **最後更新**：2026-07-24（UTC 約 01:10）  
> **程式版本**：v0.3.5（以 `/api/health` 為準）
...
| 健康 | `GET /api/health` → `{"ok":true,"version":"0.3.5"}` |
...
| 快照進度 | 下載約 **1.3%**（~329 MB）· 速度常見 **~70–100 KB/s**（百度限速波動） |
| 排隊 | nfo / srt / 其餘海報圖 |
...
| 日期 | 筆記 |
|------|------|
| 2026-07-24 | 初版；job2 ~329MB/24.83GB 下載中；v0.3.5 |
